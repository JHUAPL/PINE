...
export class AnnotateComponent implements OnInit, AfterViewInit {
	...
	public htmlWordsList: Element[];

	@ViewChild("wordsHtml")
	public wordsHtml: ElementRef;
	...
	constructor(private route: ActivatedRoute,
		...
		Window['htmlEvent'] = this.htmlEvent.bind(this);
	}
	...
	ngAfterViewInit() {
		...
		this.route.paramMap.subscribe((params: ParamMap) => {
			...
			this.documents.getDocumentDetails(docId).subscribe((doc: Document) => {
				...
				if (doc.metadata.html_view) {
					this.isHtml = true;

					let htmlDoc = new DOMParser().parseFromString(doc.metadata.html_view, 'text/xml');
					this.htmlWordsList = [];

					// get styles
					let styles = htmlDoc.getElementsByTagName('style');
					let styleStr = '';
					_.forEach(styles, (style: Element) => {
						styleStr = styleStr + style.outerHTML;
					});
					this.styleHtml = this.domSanitizer.bypassSecurityTrustHtml(styleStr);

					// get content
					let spans = htmlDoc.getElementsByTagName('span');
					_.forEach(spans, (span: Element) => {
						if (span.hasAttribute('ID') && span.getAttribute('ID').startsWith('word_')) {
							span.id = span.getAttribute('ID');
							let parts = span.id.split('_');
							let start = parts[1];
							let end = parts[2];
							span.className = span.className + ' word';
							span.setAttribute('word-start', start);
							span.setAttribute('word-end', end);
							span.setAttribute('oncontextmenu', "Window.htmlEvent('context', event, '" + span.id + "')");
							span.setAttribute('onmousedown', "Window.htmlEvent('mousedown', event, '" + span.id + "')");
							span.setAttribute('onmouseup', "Window.htmlEvent('mouseup', event, '" + span.id + "')");
							span.setAttribute('onmouseover', "Window.htmlEvent('mouseover', event, '" + span.id + "')");
							span.setAttribute('onmouseout', "Window.htmlEvent('mouseout', event, '" + span.id + "')");
							this.htmlWordsList.push(span);
						}
					});
					let bodies = htmlDoc.getElementsByTagName('body');
					let bodyStr = '';
					_.forEach(bodies, (body: Element) => {
						bodyStr = bodyStr + body.innerHTML;
					});
					/* bodyStr = bodyStr.replace(/(?<!(<\/?[^>]*|&[^;]*))([^\s<]+)/g, `<span class="word" onmousedown="Window.wordMouseDown(event)"
						onmouseover="Window.wordMouseOver(event)" onmouseup="Window.wordMouseUp(event)" onmouseout="Window.wordMouseOut(event)"
						onclick="Window.wordClick(event)" oncontextmenu="Window.contextMenu(event)">$1$2</span>`) */
					this.contentHtml = this.domSanitizer.bypassSecurityTrustHtml(bodyStr);
				} else {
					this.styleHtml = undefined;
				...
				forkJoin(this.documents.getCollectionUserPermissions(docId),
					...
					if (this.isHtml) {
						...
						this.nerData.setWordsAndAnnotations(
							Word.parseWordObjectsFromHtml(wordSpans),
	...
	private wordsLoaded() {
		if (this.isHtml) {
			let container = document.getElementById('words-html');
			console.log('container', container);
			if (!container) {
				return;
			}
			let words = container.querySelectorAll('.word');
			_.forEach(words, (wordElem: HTMLElement) => {
				const word = this.nerData.getWordById(wordElem.id);
				if (word) {
					word.elem = wordElem;
					if (word.innerHTML) {
					...
					wordElem.removeAttribute('onmouseover');
					wordElem.removeAttribute('onmousedown');
					/* span.setAttribute('word-start', start);
							span.setAttribute('word-end', end);
							span.setAttribute('oncontextmenu', "Window.htmlEvent('context', event, '" + span.id + "')");
							span.setAttribute('onmousedown', "Window.htmlEvent('mousedown', event, '" + span.id + "')");
	...
	public htmlEvent(type: 'context' | 'mousedown' | 'mouseover' | 'mouseup' | 'mouseout', event, wordId) {
		let word = this.nerData.getWordById(wordId);
		if (!word) {
			return;
		}
		switch (type) {
			case 'context':
				this.contextMenu(event, word);
				break;
			case 'mousedown':
				this.mousedown(event, word);
				break;
			case 'mouseover':
				this.mouseover(event, word);
				break;
			case 'mouseup':
				this.mouseup(event, word);
				break;
			case 'mouseout':
				this.mouseout(event, word);
				break;
		}
	}
	...
	public mouseover(event: MouseEvent, word: Word) {
		if (!this.canCurrentlyAnnotate || !word) return;
	...
	private removeAllAnnotations() {
		for (const word of this.nerData.words) {
			...
			word.elem.style.background = "transparent";
		}
